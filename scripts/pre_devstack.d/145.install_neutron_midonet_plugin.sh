...
if [ -z $MIDOSTACK_NEUTRONPLUGIN_PACKAGE_URL ] || [ -z $MIDOSTACK_MIDONETCLI_PACKAGE_URL ]; then
    ...
    wget $MIDOSTACK_MIDONETCLI_PACKAGE_URL -O /tmp/midonet-cli.deb
    sudo dpkg -i /tmp/midonet-cli.deb
    wget $MIDOSTACK_NEUTRONPLUGIN_PACKAGE_URL -O /tmp/neutronplugin.deb
    sudo dpkg -i /tmp/neutronplugin.deb
fi
