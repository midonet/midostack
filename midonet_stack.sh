...
if [ ! -z $MIDOSTACK_MIDOLMAN_PACKAGE_URL ] && [ ! -z $MIDOSTACK_MIDONETAPI_PACKAGE_URL ]; then
    echo Midolman package: $MIDOSTACK_MIDOLMAN_PACKAGE_URL
    echo Midonet API package: $MIDOSTACK_MIDONETAPI_PACKAGE_URL
else
    echo Midonet repo: $MIDONET_GIT_REPO
    echo MidoNet branch: $MIDONET_GIT_BRANCH
fi
if [ ! -z $MIDOSTACK_MIDONETCLI_PACKAGE_URL ]; then
    echo Midonet client package: $MIDOSTACK_MIDONETCLI_PACKAGE_URL
else
    echo MidoNet client repo: $MIDONET_CLIENT_REPO
    echo MidoNet client branch: $MIDONET_CLIENT_BRANCH
fi
if [ ! -z $MIDOSTACK_NEUTRONPLUGIN_PACKAGE_URL ]; then
    echo Midonet plugin package: $MIDOSTACK_NEUTRONPLUGIN_PACKAGE_URL
else
